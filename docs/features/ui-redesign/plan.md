...
- [GUESS] Frosted glass pre-API-31 fallback: spec mentions Canvas Gaussian blur on bitmap. This is non-trivial. Plan defers the fallback to Phase 7 polish — API 31+ only for initial implementation (Min SDK is 34, so this is actually fine — no fallback needed).
- [GUESS] Palette-ktx dependency: not currently in the project. Will need to be added to version catalog.
